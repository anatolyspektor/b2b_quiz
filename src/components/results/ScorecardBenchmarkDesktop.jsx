...
export default function ScorecardBenchmarkDesktop({
  ...
  return (
    <div className="w-full max-w-5xl mx-auto rounded-xl bg-[#0f373c] text-white shadow-lg p-6 md:p-8 space-y-6">
      <h3 className="sm:mb-20 sm:text-6xl lg:text-5xl font-bold text-center">
        Dependency Score
      </h3>
      ...
      {/* Result Text */}
      {preview ? (
        <div className="blur-sm select-none space-y-6">
          <p className="text-2xl text-center flex flex-wrap justify-center gap-2 sm:mt-20 sm:text-4xl">
            You scored
            <span
              className={`inline-flex items-center justify-center w-15 h-10 rounded-full text-[#0f373c] font-bold ${zoneBg}`}
            >
              {score}
            </span>
            , which puts you in the
            <span
              className={`px-2 py-0.5 rounded text-lg font-bold sm:text-3xl ${zoneBg}`}
            >
              {zone}
            </span>
            zone.
